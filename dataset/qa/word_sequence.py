...
if not os.path.exists(QA_BAIKE_SEQUENCE_VOCAB):
    ws = WordSequence()
    ...
    pickle.dump(ws, open(QA_BAIKE_SEQUENCE_VOCAB, 'wb'))
    ...
    pickle.dump(ws, open(QA_BAIKE_SEQUENCE_BY_WORD_VOCAB, 'wb'))
...
def get_ws(by_word=True):
    if by_word:
        pickle.load(open(QA_BAIKE_SEQUENCE_BY_WORD_VOCAB, 'rb'))
    else:
        return pickle.load(open(QA_BAIKE_SEQUENCE_VOCAB, 'rb'))
