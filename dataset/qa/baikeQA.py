from dataset.qa.word_sequence import get_ws
...
class QASets(Dataset):

    def __init__(self):
        super(QASets, self).__init__()

        self.inputs = open(QA_QUESTIONS, encoding='utf-8').readlines()
        self.targets = open(QA_SIM_QUESTIONS, encoding='utf-8').readlines()
        # total = len(inputs)
